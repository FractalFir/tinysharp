...
impl Method {
    ...
    fn resolve_node(&mut self, index:usize,parrent_state:StackState)-> Result<(), MethodIRError>{
        ...
        match link{
            ...
            BlockLink::Pass=>{
                ...
                self.resolve_node(child_index,self.blocks[index].state().expect("State did not resolve, but no error raised!"))
            },
            BlockLink::Branch(default,target)=>{
                let def_index = self.get_index_of_block_beginig_at(default);
                self.resolve_node(def_index,self.blocks[index].state().expect("State did not resolve, but no error raised!"))?;
                let target_index = self.get_index_of_block_beginig_at(target);
                self.resolve_node(target_index,self.blocks[index].state().expect("State did not resolve, but no error raised!"))
            },

use super::{op::OpKind, BlockLink, InstructionIndex, MethodIRError, Signature, StackState, VOp,method::Method};
...
impl OpBlock {
    ...
    pub (crate) fn is_resolved(&self)->bool{self.state_change.is_some()}
    ...
    pub (crate) fn into_llvm_bb(&self,meth:&Method,block_builder:&mut Builder,ctx:&Context){
        assert!(self.state_change.clone().expect("Can't convert unresolved block to LLVM IR").is_empty(),"Cross-Block Virtual stack is not supported yet");
        ...
        for op in &self.block{
            match op.kind(){
                OpKind::Ret=>{
                    let ret = op.resolved_type().expect("Unresolved type during code gen.");
                    if let Type::Void = ret{
                        block_builder.build_return(None);
                    }
                    else{
                        let top_any = (&vars[vstack.pop().expect("Noting to return on stack!")]).as_any_value_enum();
                        let top:BasicValueEnum = top_any.try_into().expect("Nonreturnable value on the stack!");
